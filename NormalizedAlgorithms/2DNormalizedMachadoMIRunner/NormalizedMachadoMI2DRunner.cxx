...
// Example adpted from ITK registration examples;
// We have a simple 2D rigid registration using the Normalized
// implementation of the Tsallis Mutual Information.
...
int main( int argc, char *argv[] )
{
  ...
  ShrinkFilterType::Pointer shrinkFilterFixed = ShrinkFilterType::New();
  shrinkFilterFixed->SetInput(fixedImageReader->GetOutput() );
  shrinkFilterFixed->SetShrinkFactor(1, 3);
  shrinkFilterFixed->Update();

  ShrinkFilterType::Pointer shrinkFilterMoving = ShrinkFilterType::New();
  shrinkFilterMoving->SetInput(movingImageReader->GetOutput());
  shrinkFilterMoving->SetShrinkFactor(1, 3);
  shrinkFilterFixed->Update();

  registration->SetFixedImage(    shrinkFilterFixed->GetOutput()    );
  registration->SetMovingImage(   shrinkFilterMoving->GetOutput()   );
